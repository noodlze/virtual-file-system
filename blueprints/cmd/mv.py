...
from models.utils import delete_item, item_exists, move_item, with_row_locks
...
def validate_mv_cmd_args(match):
...
def execute_mv_cmd(mv_args):
    ...
    if len(mv_args.srcs) == 1:
        ...
        dest_exists, dest_item_id = item_exists(mv_args.abs_dest)
        if dest_exists:
            ...
            if dest_item_is_dir:
                ...
                move_item(mv_args.srcs_id[0], dest_item_id)
            else:  # else dest_item = file
                # delete destination file
                delete_item(dest_item_id)
                # insert new file in the same parent directory as the deleted destination file
                move_item(mv_args.src_id[0], parent_dest_item_id)
        else:
